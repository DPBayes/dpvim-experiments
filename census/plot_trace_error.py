...
##########################################
# read the data
df_whole = pd.read_csv("USCensus1990.data.txt", index_col=0)
...
model, _, preprocess_fn, postprocess_fn = load_custom_numpyro_model("model_adjusted.py", args, unknown_args, train_df_whole)
...
def load_traces_for_run(path, prefix, epsilon, seed, clipping_threshold, args):
    if epsilon == "non_dp":
        wholepop_output_name = filenamer(prefix, "wholepop", args, epsilon=epsilon, seed=seed, clipping_threshold=clipping_threshold)
    ...
    poisson_regression_keys = ["intercept"] + [key for key in loc_trace.keys() if "weight" in key]
...
if args.plot_traces:
    ...
    plot_name = f"avg_trace_error_{args.epsilon}_census_"
    ...
    for prefix in PREFIXES:
        ...
        axis.errorbar(
                ...
                alpha=1.0, label=prefix
        )

    axis.set_xlabel("current epoch")
    #axis[0].set_ylabel("MPAE for mean")
    axis.set_ylabel("MPAE for std")
    handles, labels = axis.get_legend_handles_labels()
    axis.legend(handles, labels)
    axis.grid(alpha=0.5, lw=1)


    fig.suptitle(f'Evolution of error in learned parameters\n$\\varepsilon=${args.epsilon}')
