...
from dpsvi import AlignedNaturalGradientDPSVI

"""
Inference script for testing the data generating model with the natural gradient DPVI
"""
...
class InferenceException(Exception):
    ...
    def __init__(self, trace_tuple: traces, grad_trace_tuple: traces):
        self.traces = trace_tuple
        self.grad_traces = grad_trace_tuple
...
def _train_model(rng, rng_suite, svi, data, batch_size, num_data, num_epochs, silent=False):
    ...
    grads_template = jax.tree_map(lambda x: jnp.zeros((batch_size, *x.shape)), params)

    @jax.jit
    def train_epoch(num_iters_for_epoch, svi_state, batchify_state):
        def update_iteration(i, state_and_loss):
            svi_state, loss, _ = state_and_loss
            batch = get_batch(i, batchify_state)
            svi_state, iter_loss, grads = svi.update(svi_state, *batch)
            return (svi_state, loss + iter_loss / num_iters_for_epoch, grads)

        return jax.lax.fori_loop(0, num_iters_for_epoch, update_iteration, (svi_state, 0., grads_template))
    ...
    for e in progressbar:
        ...
        svi_state, loss, grads = train_epoch(num_batches, svi_state, batchify_state)
        ...
        if e == 0:
            ...
            loc_grads_over_epochs = np.zeros((num_epochs, *grads['auto_loc'].shape))
            scale_grads_over_epochs = np.zeros((num_epochs, *grads['auto_scale'].shape))
        locs_over_epochs[e] = params_after_epoch['auto_loc']
        scales_over_epochs[e] = params_after_epoch['auto_scale']
        loc_grads_over_epochs[e] = grads['auto_loc']
        scale_grads_over_epochs[e] = grads['auto_scale']
        if np.isnan(loss):
            raise InferenceException(
                traces(locs_over_epochs, scales_over_epochs),
                traces(loc_grads_over_epochs, scale_grads_over_epochs)
            )
        loss /= num_data
        progressbar.set_description(f"epoch {e}: loss {loss}")

    return svi.get_params(svi_state), loss, \
        traces(locs_over_epochs, scales_over_epochs), traces(loc_grads_over_epochs, scale_grads_over_epochs)
...
def main():
    ...
    parser.add_argument("--save_grad_traces", action='store_true', default=False, help="Save unclipped, unperturbed per-example gradient traces")
    ...
    output_name = filenamer("aligned_ng_adam-3", args)
    output_path = f"{os.path.join(args.output_dir, output_name)}"

    # train model
    optimizer = Adam(1e-3)
    # optimizer = SGD(1e-1)
    svi = AlignedNaturalGradientDPSVI(
        model,
        guide,
        optimizer,
    ...
    try:
        posterior_params, elbo, trace_tuple, grad_trace_tuple = _train_model(
        ...
        print("FAILED: ")
        print(e)
        trace_tuple = e.traces
        grad_trace_tuple = e.grad_traces

    if args.save_traces:
        pickle.dump(trace_tuple, open(f"{output_path}_traces.p", "wb"))

    if args.save_grad_traces:
        pickle.dump(grad_trace_tuple, open(f"{output_path}_grad_traces.p", "wb"))
